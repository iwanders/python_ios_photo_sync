...
class Storage:
    ...
    def metadata_for_path(asset):
        ...
        for suffix, key in (("create", "creation_date"), ("mod", "modification_date")):
            t = asset[key]
            for f in formats:
                z[f + "_" + suffix] = datetime.datetime.utcfromtimestamp(t).strftime(
                    "%" + f
                )
        return z
    ...
    def retrieve(self, p, asset):
        ...
        path_to_metadata = self.get_metadata_path(asset)
        logger.debug(
            f"Retrieving id: {asset['local_id']} modified at {asset['modification_date']}"
        )
        retrieved = p.retrieve_asset_by_local_id_or_burst(asset)
        ...
        if len(z) != retrieved["_filesize"]:
            raise BaseException(
                f"File size incorrect for {get_path}, expected {len(z)}."
            )
        ...
        if h != expected:
            raise BaseException(
                f"Md5 does not match! Got {h} for {get_path}, expected {expected}"
            )
...
def run_sync(args):
    ...
    logger.debug(f" save-full-burst: {args.save_full_burst}")

    p = Phone(args.host)
    sync = Storage(dir=args.dir, path=args.path, metadata_path=args.metadata_path)
    if args.save_full_burst:
        on_phone = p.get_all_metadata_with_burst()
    else:
    ...
    for i, asset in enumerate(to_sync):
        ...
        date = datetime.datetime.utcfromtimestamp(retrieved["creation_date"]).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        logger.info(
            f"{i + 1: >5} / {total: >5}: {filename: >20} {date} ({size: >9} bytes)"
        )
...
def run_test(args):
    ...
    r = p.client.retrieve_asset_by_local_id_or_burst(d[-1])
    print(r)
    ...
    for asset in on_phone:
        ...
        if staleness >= args.retain_duration:
            ...
            if asset["local_id"] in keep_photos:
                logger.debug(
                    f"  Preserving {asset['local_id']}  {asset['filename']} because in keep."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--verbosity",
        action="count",
        help="Increase verbosity,nothing is warn/error only, -v is info, -vv is debug.",
        default=0,
    )

    parser.add_argument(
        "--host",
        default="http://$REPL_HOST:1338",
        help="xmlrpc interface to connect to. Defaults to %(default)s",
    )
    ...
    def add_storage_args(parse):
        parse.add_argument(
            "--dir", default="/tmp/storage", help="Directory to write output to."
        )
        parse.add_argument(
            "--path",
            default="{Y_create}-{m_create}/{filename}",
            help="Format to use when writing.",
        )
        parse.add_argument(
            "--metadata-path",
            default="{Y_create}-{m_create}/metadata/{filename}",
    ...
    sync_parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If set, save all photos from bursts",
    )
    ...
    def sane_date_parser(v):
        ...
        if v.endswith("d"):
            ...
            raise BaseException(
                "Date should end with 'd' for days, 'w' for weeks, 'm' for months"
            )
    ...
    delete_parser = subparsers.add_parser(
        "delete",
        help="Remove files older than given duration and not in a manually created album.",
    )
    add_storage_args(delete_parser)
    delete_parser.add_argument(
        "--retain-duration",
        default="30d",
        type=sane_date_parser,
        help="Duration to keep. Default: %(default)s, d=day, w=week, m=month",
    )
    delete_parser.add_argument(
        "--ignore-integrity",
        default=False,
        action="store_true",
        help="Skip the integrity check.",
    )
    ...
    if "REPL_HOST" in os.environ:
        ...
        logger.warning(
            "no REPL_HOST set, you probably want to set this or pass --host to set the hostname"
        )
